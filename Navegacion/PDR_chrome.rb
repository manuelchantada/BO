...
def usuario(browser)
  ...
  if browser.div(:class=>"panel1").exists?
    if (browser.div(:class=>"panel1").text.slice(0..5)==user && (browser.text_field :name=>'ANI').exists?)
    else
      if (browser.div(:class=>"panel1").text=~/Iniciar/)!=nil
        ingreso browser,user,pass
    ...
    if (browser.text_field :name=>'ANI').exists?
    else
      ingreso browser,user,pass
      browser.goto "https://portalderelaciones.tasa.telefonica.com.ar/wps/myportal/!ut/p/b1/hc6xDoIwGATgJzK9toLtWKCUhgAaq2AXw2AMiYCD8fkF46RR_-2S7y4_8aQhfmjv3bm9dePQXubsw2NoSscKx2xSxCtYKYVzwR4QfAKHCeDLKfzr18Q_iQTPIys5ql3KYdUmy0qtOeLgHYAlMxCUplUIrPGxYPJoAoYVeusosHyBH0-W2difyLVv0NmFegAEiCiT/dl4/d5/L2dJQSEvUUt3QS80SmtFL1o2XzZHTlQyTVQySURERUYwSUwxQTkwTkszMDA3/"
    end
...
#Elmementos por ID
#Captura Recomendacion y Hs de recomendacion
...
def captura(browser,num,capt) 
  ...
  #Captura Recomendacion de Clearview
  def capturareco(browser)
    reco=browser.div(:id=> "v-Z7_IGHE1J80J8L270AKC3SB4N1006").text
    g=reco=~/Recomend/
    if g!=nil
      recohs=reco.slice(g+15..g+19)
        ...
        if reco.grep(/Chequee si hay falta de microfiltro/).any?
          ...
          if reco.grep(/Se recomienda Despacho/).any?
            ...
            if reco==""
              ...
              return "3,"+"00:00,"
              log.puts num.slice(0..9)+" - "+reco.to_s
            end
          end
        end
      end
  ...
  until cont>2 or (sincro!=0 && conex!=0) do     
    ...
    browser.driver.manage.timeouts.implicit_wait = 90
    Watir::Wait.until(90) {(browser.div :class=>'v-table-caption-container v-table-caption-container-align-center').exists?}
    sleep(5)

    #Captura sincro, resincro y estabilidad.
    sincro=capturasincro browser
    reco=capturareco browser
    ...
    if (sincro==0 or conex==0)
      ...
      if cont>2
        ...
        if (browser.div :id=>'ibox_footer').text=="Informaci\u00F3n"
          (browser.a :href=>'javascript:void(0)').click
          sleep(2)
        end
