...
begin
	...
	control=File.new("C:\\macros\\tsc\\control_line.txt","a+")
	if control.eof?
		lote=File.new(lote2)
		else
		ultima=control.readline
	    lote=File.new(lote2)
		lote.each_line do |li|
		break if (li[ultima])
		end
	end
	control.close	
...
while  Time.now.strftime("%H").to_i>=23 
	puts " * * * waiting working hour * * * "
	sleep(20000)
end
...
control=File.new("C:\\macros\\tsc\\control_line.txt","w")
control.write @linea
control.close
